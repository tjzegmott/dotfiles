...
function M.create_or_update_task()
  ...
  local line_length = vim.fn.strlen(vim.fn.getline("."))
  ...
  for _, keyword in ipairs(keywords) do
    ...
    if start_index then
      local task_description = string.sub(current_line, end_index + 2, line_length)
...
function M.go_to_task_in_taskwarrior_tui()
  ...
  for _, keyword in ipairs(patterns) do
    local start_index, end_index = string.find(current_line, keyword)
    if start_index then
      taskDescription = string.sub(current_line, end_index + 1, line_length)
      taskDescription = string.gsub(taskDescription, "%(piotr1215%)", "")
      break
    end
  end
  ...
  if taskDescription then
    ...
    local output =
      vim.fn.system(" ~/.config/taskwarrior-tui/shortcut-scripts/switch_to_tui.sh '" .. taskDescription .. "'")
  end
end
